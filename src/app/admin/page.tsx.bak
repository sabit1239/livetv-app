...
import {
  Shield, Tv, Users, TrendingUp, Plus, Edit, Trash2,
  Search, BarChart3, Settings, LogOut, Eye, RefreshCw
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'channels' | 'users'>('dashboard');
  ...
  const stats = [
    { label: 'Total Channels', value: channels.length, icon: Tv, color: 'text-blue-400', bg: 'bg-blue-400/10' },
    { label: 'Sports', value: channels.filter((c) => c.category === 'sports').length, icon: TrendingUp, color: 'text-green-400', bg: 'bg-green-400/10' },
    { label: 'Featured', value: channels.filter((c) => c.isFeatured).length, icon: Eye, color: 'text-yellow-400', bg: 'bg-yellow-400/10' },
    { label: 'Categories', value: new Set(channels.map((c) => c.category)).size, icon: BarChart3, color: 'text-purple-400', bg: 'bg-purple-400/10' },
  ];
  ...
  const handleEdit = (channel: Channel) => {
    ...
    setFormData({
      ...
      category: channel.category, country: channel.country || '',
      language: channel.language || '', description: channel.description || '',
      isHD: channel.isHD || false, isLive: channel.isLive || false,
      isFeatured: channel.isFeatured || false, isTrending: channel.isTrending || false,
    });
    setShowAddForm(true);
  };
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/50 mb-6">You need admin privileges to access this page.</p>
    ...
    <div className="min-h-screen bg-black">
      <div className="max-w-screen-2xl mx-auto px-4 md:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/50 text-sm">Manage channels and content</p>
            </div>
          </div>
          <Link href="/" className="text-white/50 hover:text-white text-sm flex items-center gap-2 transition-colors">
            ← Back to Site
          </Link>
        </div>
        ...
        <div className="flex gap-2 mb-6 border-b border-white/10 pb-0">
          {[['dashboard', 'Dashboard', BarChart3], ['channels', 'Channels', Tv], ['users', 'Users', Users]].map(([id, label, Icon]) => (
            <button
              key={id}
        ...
        {activeTab === 'channels' && (
          <div>
            ...
            {/* Add/Edit Form */}
            {showAddForm && (
              <div className="bg-zinc-900 border border-white/10 rounded-2xl p-6 mb-6 animate-slide-down">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setFormData({ ...formData, category: e.target.value as Channel['category'] })}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full bg-zinc-800 border border-white/10 text-white placeholder-white/20 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-red-500 resize-none"
                    />
                  </div>
                ...
                <div className="flex gap-3 mt-5">
                  ...
                  <button onClick={resetForm} className="bg-zinc-800 hover:bg-zinc-700 text-white/70 hover:text-white px-6 py-2.5 rounded-xl text-sm font-semibold transition-colors">
                    Cancel
                  </button>
                </div>
              </div>
            ...
            <div className="space-y-2">
              {filteredChannels.map((channel) => {
                ...
                return (
                  <div key={channel.id} className="flex items-center gap-4 bg-zinc-900 border border-white/5 hover:border-white/10 rounded-xl p-4 transition-all group">
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => router.push(`/channel/${channel.id}`)} className="p-2 text-white/40 hover:text-white hover:bg-white/10 rounded-lg transition-all" title="Preview">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleEdit(channel)} className="p-2 text-white/40 hover:text-blue-400 hover:bg-blue-400/10 rounded-lg transition-all" title="Edit">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDelete(channel)} className="p-2 text-white/40 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all" title="Delete">
        ...
        {activeTab === 'dashboard' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${(count / channels.length) * 100}%` }}
                          />
                        </div>
                      </div>
                    </div>
            ...
            <div className="bg-zinc-900 border border-white/10 rounded-xl p-6">
              ...
              <div className="space-y-3">
                {[
                  { label: 'Add New Channel', icon: Plus, action: () => { setActiveTab('channels'); setShowAddForm(true); } },
                  { label: 'Browse All Channels', icon: Tv, action: () => router.push('/search') },
                  { label: 'Import Playlist', icon: RefreshCw, action: () => router.push('/settings?tab=playlists') },
        ...
        {activeTab === 'users' && (
          <div className="text-center py-16">
            ...
            <p className="text-white/40 text-sm">Connect Firebase Admin SDK to manage users</p>
          </div>
        )}
      </div>
    </div>
